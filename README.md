...
frontend/                 Next.js 静态前台
  ...
  tests/                  前端构建产物测试
admin-frontend/           独立 React + Three.js 管理前端
  src/                    盲盒配置、动作预览和发布工作台
...
- 自有服务器：后端执行 `deploy/deploy-backend.sh`；公开前端使用 `deploy/deploy-frontend.ps1`；管理前端使用 `deploy/deploy-admin-frontend.ps1`。
